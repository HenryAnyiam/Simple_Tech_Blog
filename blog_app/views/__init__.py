from .article_views import ArticleCreateView, ArticleDraftView, ArticleView, ArticleDetailView, ArticleDeleteView
from .home_view import HomeView
from .comment_view import CommentCreateView
from .like_views import LikePostView
from .user_views import CreateUserView, CreateProfileView, ConfirmUserView, LoginView, LogoutView
from .user_views import UserDetailView, UserEditView
from .user_views import ResetPassword, ForgotPassword
from .news_views import NewsList